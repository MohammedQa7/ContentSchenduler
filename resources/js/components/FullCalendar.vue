<template>
    ...
    <div class="rounded-lg border shadow-sm p-6 m-4 max-w-full ">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 ">
            <div>
                <h1 class="text-2xl font-bold text-foreground">Date</h1>
        ...
        <div v-if="currentView === 'week'" class="overflow-hidden">
            ...
            <div class="grid grid-cols-7 gap-2 mt-4">
                <div v-for="(day, dayIndex) in weekDays" :key="dayIndex"
                    ...
                    <div v-for="scheduledPost in getPostsForDate(day.date)" :key="`${dayIndex}-${scheduledPost.id}`"
                        class="mb-2 p-2 rounded-lg cursor-pointer text-sm space-y-2 transition-all hover:shadow-md bg-background border border-border text-foreground hover:bg-muted/20">
                        <div class="font-medium">{{ scheduledPost.title }}</div>
                        <Badge v-for="(platform, index) in scheduledPost.platforms" :key="index">
                            {{ platform.name }}
                        </Badge>
                        <slot name="week"/>
                    </div>
        ...
        <div v-else-if="currentView === 'month'" class="overflow-hidden">
            ...
            <div class="grid grid-cols-7 gap-1">
                <div v-for="(day, index) in monthDays" :key="index"
                    ...
                    <div v-for="scheduledPost in getPostsForDate(day)" :key="`month-${index}-${scheduledPost.id}`"
                        class="mb-1 px-1.5 py-1 rounded text-xs cursor-pointer truncate transition-colors  bg-accent border-l-4 border-l-primary/30 text-foreground"
                        @click="selectEvent(scheduledPost)">
                        {{ scheduledPost.title }}
                    </div>
                </div>
            </div>
...
const getPostsForDate = (calendarDate) => {
    const fomratedCalendarDate = new Date(calendarDate).toISOString();

    return propsData.posts.filter(post => {
        return dayjs(fomratedCalendarDate).isSame(dayjs(post.scheduledTime), 'day')
    });
};
